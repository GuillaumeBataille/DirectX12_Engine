...
D3DApp* D3DApp::mApp = nullptr; // The only one D3DApp instance
D3DApp* D3DApp::GetApp()		
{
	return mApp;
}

/* Init the unique D3DApp instance and set the application instance handle */
...
int D3DApp::Run()
{
	...
	mTimer.Reset(); // Starting GameTimer
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if (!mAppPaused)
			{
				CalculateFrameStats();	// Update frame statistics
				Update(mTimer);			// Update the timer
				Draw(mTimer);			// Draw
			}
			else
			{
...
{
	if (!InitMainWindow())	// Try to init the window
		return false;

	if (!InitDirect3D())	// Try to init Direct3D
		return false;	
...
{	
	// Verify if the device, the swapchain and the allocator exist
	assert(md3dDevice);
	assert(mSwapChain);
	...
	// Clear each all swapchain buffer & clear the Depth stencil buffer
	...
	mCurrBackBuffer = 0; //Set the currentBackbuffer to the id 0 

	// Recreate a new RTV for each swap chain buffer
	...
	// Create depth stencil view with the buffer
	...
	// Transform the state of the buffer from common to depthwrite to consider it properly
